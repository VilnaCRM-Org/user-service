...
| Unsure about a Symfony, API Platform, Doctrine ODM, or library API    | `context7`             | Context7 MCP (`resolve-library-id` → `get-library-docs`) instead of guessing from memory               |
...
| Committing, pushing, or opening a PR                                  | `commit-commands`      | `/commit-commands:commit` or `/commit-commands:commit-push-pr`                                         |
