...
| `testing-workflow`         | Applied. Added Bats coverage for review prompt criteria, applicability guardrails, NFR checklist retention, fix prompt scoping, stale-base regression, and Claude policy parity. Focused Bats passed with 7 tests and the full Bats helper suite passed with 94 tests.                                                                                                                       |
...
- `bats -f "review prompt requires|review prompt prevents|review prompt retains|fix prompt constrains|claude agent receives|prefers origin base|local base branch before" tests/CLI/bats/make_ai_review_loop_tests.bats`: passed, 7 tests.
- `bats tests/CLI/bats/make_ai_review_loop_tests.bats tests/CLI/bats/make_bmalph_tests.bats`: passed, 94 tests.
- `make ci`: passed after the final Bats/doc updates.
- `make ai-review-loop`: passed after final `make ci`; Codex reviewer reported `STATUS: PASS` and `0 issues`.
