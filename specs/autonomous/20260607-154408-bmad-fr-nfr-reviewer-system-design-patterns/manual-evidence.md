...
| Local AI review             | Run `make ai-review-loop` with `AI_REVIEW_BASE=main` and isolated port variables after full CI. | Codex reviewer returned `STATUS: PASS` with `0 issues`; no fixer phase ran.                                                                                                                                                                                                                                                |
...
git diff --check
bats -f "review prompt requires|review prompt prevents|review prompt retains|fix prompt constrains|claude agent receives|prefers origin base|local base branch before" tests/CLI/bats/make_ai_review_loop_tests.bats
...
- Focused Bats regression set: PASS, 7 tests.
- Full Bats helper suite: PASS, 94 tests.
...
- `make ci`: PASS, including unit tests (2258 tests, 6208 assertions),
  ...
  killed.
- `make ai-review-loop`: PASS after final CI; Codex reviewer reported
