...
- `web-auth/webauthn-lib` covers core WebAuthn validation, so the service does not hand-roll cryptographic verification.
- Existing `IssuedSessionFactory` can issue tokens after successful passkey sign-up/sign-in.

## Watch Items

- CI may reject large service methods through PHPMD; keep orchestration small and split factories/verifiers.
