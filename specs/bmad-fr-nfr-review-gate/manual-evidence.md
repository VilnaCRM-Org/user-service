...
| Whole-codebase impact markers   | Run Bats fake-agent tests for BMAD wrapper impact markers and scorecard validation.                                    | PASS requires `WHOLE_CODEBASE_IMPACT: PASS`, `GRAPH_IMPACT_CONTEXT: PASS`, `IMPACT_ANALYSIS_MIN_SCORE: 5/5`, graph evidence, and every pinned impact surface.  | FR-15, FR-17, AC-17  |
...
| Dependency security remediation | Merge current dependency baseline from `origin/main` and run security verification in the PHP container.               | Current `composer.lock` includes the all-deps security baseline; `make check-security` reports no known vulnerabilities.                                       | FR-14, AC-15, NFR-04 |
...
| Uncommitted PASS guard          | Run Bats fake-agent test `ai-review-loop rejects BMAD PASS when fixes are only in the dirty worktree`.                 | Final PASS publication is rejected when the reviewed fixes exist only as local uncommitted worktree content.                                                   | AC-11, AC-12, NFR-03 |
...
- `composer.lock`: CI/security remediation from a scoped Composer update within
  existing `composer.json` constraints; `composer.json` is unchanged.
