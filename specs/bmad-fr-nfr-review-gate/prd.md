...
| FR-02 | The gate passes the spec path, manual evidence path, PR number, base ref, score threshold, NFR categories, expanded quality dimensions, and whole-codebase impact context into the review prompt.                                                  | P0       |
...
| FR-05 | The gate requires 5/5 for every applicable FR, NFR, NFR catalog category, expanded quality dimension, whole-codebase impact surface, manual-test requirement, QA checkpoint, GitHub gate, and CI gate.                                             | P0       |
...
| FR-17 | The gate can pass optional Graphify, codebase-memory, Deptrac, or equivalent knowledge-graph impact context to the reviewer without making those tools hard runtime dependencies.                                                                  | P1       |
...
| NFR-10 | Analysability    | The reviewer receives changed-file and optional graph-context evidence so it can trace affected callers, contracts, data paths, tests, docs, and operational surfaces. |
| NFR-11 | Portability      | Whole-codebase impact analysis works without a mandatory graph tool; Graphify/codebase-memory/Deptrac outputs are optional accelerators.                               |
...
| AC-04 | The BMAD review prompt receives substituted spec, manual evidence, PR, base ref, threshold, category, expanded-quality, impact-surface, status, and impact-context values.                     |
| AC-05 | A fake reviewer PASS with all required markers, including `EXPANDED_QUALITY_SCORECARD: PASS`, `WHOLE_CODEBASE_IMPACT: PASS`, and `CI_GATE: PASS`, exits successfully when verification passes. |
...
| AC-18 | When no impact context is supplied, the wrapper creates a bounded changed-file and graph-context handoff file for the reviewer.                                                                |
