...
final class OAuth2ScopeType extends Type
{
    ...
    public function convertToDatabaseValue(mixed $value): ?array
    {
        ...
        if (!is_array($value)) {
            throw new InvalidArgumentException('OAuth2ScopeType expects an array of stringable values.');
        }
    ...
    {
        return 'if ($value === null) { $return = null; } elseif (is_array($value)) { $return = []; foreach ($value as $item) { if (is_string($item) || (is_object($item) && method_exists($item, "__toString"))) { $return[] = (string) $item; } else { throw new \InvalidArgumentException("OAuth2ScopeType expects an array of stringable values."); } } } else { throw new \InvalidArgumentException("OAuth2ScopeType expects an array of stringable values."); }';
    }
