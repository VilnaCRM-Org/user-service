...
| NFR-06 Email verification           | ST-2.1, ST-2.2    |
...
- AC-02: `OAuthProvider` enum supports `github` and `google`.
- AC-03: MongoDB mapping defines unique `(provider, provider_id)` and unique `(user_id, provider)` indexes.
- AC-04: Unit tests validate provider parsing and invalid-provider behavior.
...
- AC-06: Unit tests cover happy path and all failure paths.
...
- AC-06: Unit tests cover success and failures.
...
- AC-06: Unverified provider email returns RFC 7807 (`unverified_provider_email`, 422).
- AC-07: Provider outage/failure returns RFC 7807 (`provider_unavailable`, 503).
- AC-08: Success payload matches auth shape used by password flow.
- AC-09: Callback response includes `Cache-Control: no-store` and `Pragma: no-cache`.
...
- AC-01: Provider adapters cover success, unverified email, token exchange errors, and retry behavior.
- AC-02: Handlers cover all successful and failure branches.
- AC-03: OAuth-specific unit coverage >= 95%.
...
- AC-01: New user social sign-up path succeeds and emits creation event.
