...
final class PasswordResetEmailFactoryTest extends UnitTestCase
{
    ...
    {
        parent::setUp();
        $this->eventFactoryMock = $this->createMock(PasswordResetEmailSendEventFactoryInterface::class);
    }
    ...
    public function testCreate(): void
    {
        ...
        $token = new PasswordResetToken($user->getId(), 'abc123token');
    ...
    {
        $userFactory = new UserFactory();
        $user = $userFactory->create('another@example.org', 'AB', 'password123', new Uuid('456e7890-e89b-12d3-a456-426614174001'));
        $token = new PasswordResetToken($user->getId(), 'xyz789token');
        $factory = new PasswordResetEmailFactory($this->eventFactoryMock);
