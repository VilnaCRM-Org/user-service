...
@test "BMALPH setup rewrites planning artifacts to specs" {
  run bash -lc '
    ...
    cat > "$tmpdir/_bmad/config.yaml" <<'"'"'EOF'"'"'
...
@test "BMALPH wrapper skills explain how to bootstrap missing local assets" {
  run bash -lc '
    ...
    ! rg -F "or \`bmalph upgrade --force\`" .agents/skills
    exit "$missing"
  '
  assert_success
}
...
@test "documentation points to BMALPH and current-session planning" {
  run bash -lc '
    ...
    grep -F "planning artifacts under \`specs/\`" README.md
    grep -F "planning artifacts under \`specs/\`" docs/getting-started.md
    grep -F "planning artifacts under \`specs/\`" docs/onboarding.md
    grep -F "planning artifacts under \`specs/\`" AGENTS.md
    grep -F "planning artifacts under \`specs/\`" CLAUDE.md
